...
public class ConsulterImprimante extends HttpServlet {
	/**
	 * 
	 */
	...
	public void init() throws ServletException {
		...
		this.m = ((Factory) getServletContext().getAttribute(CONF_FACTORY)).getImprimanteManager();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		ImprimanteForm form = new ImprimanteForm(m);

		/* Traitement de la requ�te et r�cup�ration du bean en r�sultant */
		Imprimante metier = form.consulterImprimante(request);
		...
		if (form.getErreurs().isEmpty()) {
			...
			/* Ensuite r�cup�ration de la map dans la session */
			Map<Long, Imprimante> metiers = (TreeMap<Long, Imprimante>) session.getAttribute(SESSION_METIERS);
			/*
			 * Si aucune map n'existe, alors initialisation d'une nouvelle map
			 */
			if (metiers == null) {
				metiers = new TreeMap<Long, Imprimante>();
			}
