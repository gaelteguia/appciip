...
public class ConsulterOutil extends HttpServlet {
	/**
	 * 
	 */
	...
	public void init() throws ServletException {
		...
		this.m = ((Factory) getServletContext().getAttribute(CONF_FACTORY)).getOutilManager();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		OutilForm form = new OutilForm(m);

		/* Traitement de la requ�te et r�cup�ration du bean en r�sultant */
		Outil metier = form.consulterOutil(request);
		...
		if (form.getErreurs().isEmpty()) {
			...
			/* Ensuite r�cup�ration de la map dans la session */
			Map<Long, Outil> metiers = (TreeMap<Long, Outil>) session.getAttribute(SESSION_METIERS);
			/*
			 * Si aucune map n'existe, alors initialisation d'une nouvelle map
			 */
			if (metiers == null) {
				metiers = new TreeMap<Long, Outil>();
			}
