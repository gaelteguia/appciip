...
public class AssocierAdresseService extends HttpServlet {
	/**
	 * 
	 */
	...
	public static final String ATT_LOCALISATION = "localisation";
	...
	public static final String VUE_SUCCES = "/WEB-INF/ConsulterAdresse.jsp";
	public static final String VUE_FORM = "/WEB-INF/AssocierAdresseService.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Adresse localisation = form.associerService(request);

		/* Ajout du bean et de l'objet m�tier � l'objet requ�te */
		request.setAttribute(ATT_LOCALISATION, localisation);
		...
		if (form.getErreurs().isEmpty()) {
			...
			Map<Long, Service> services = (HashMap<Long, Service>) session.getAttribute(SESSION_SERVICES);
			/*
			 * Si aucune map n'existe, alors initialisation d'une nouvelle map
			 */
			if (services == null) {
			...
			/* Ensuite r�cup�ration de la map des commandes dans la session */
			Map<Long, Adresse> localisations = (HashMap<Long, Adresse>) session.getAttribute(SESSION_LOCALISATIONS);
			/*
			 * Si aucune map n'existe, alors initialisation d'une nouvelle map
			 */
			if (localisations == null) {
				localisations = new HashMap<Long, Adresse>();
			}
			/* Puis ajout de la commande courante dans la map */
			...
			/* Ensuite r�cup�ration de la map des commandes dans la session */
			Map<Long, Fonction> metiers = (HashMap<Long, Fonction>) session.getAttribute(SESSION_METIERS);
			/*
			 * Si aucune map n'existe, alors initialisation d'une nouvelle map
			 */
			if (metiers == null) {
				metiers = new HashMap<Long, Fonction>();
			}
